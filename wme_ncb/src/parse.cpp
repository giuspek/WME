...
void Parser::update_heuristics (long double weight) {
  if (weight > 0)
    internal->w_heur_neg = false;
  if (weight < 0)
    internal->w_heur_pos = false;
  if (weight < 1)
    internal->w_heur_pos_high = false;
  if (weight < 0 || weight > 1)
    internal->w_heur_prob = false;
}

const char *Parser::parse_weights_file() {
  int ch;
  ...
  while ((ch = parse_char()) != EOF) {
    ...
    if (!isdigit(ch))
      PER("expected digit after optional '-'");

    lit = ch - '0';
    while (isdigit(ch = parse_char())) {
      int digit = ch - '0';
      if (lit > (INT_MAX - digit) / 10)
        PER("literal too large");
      lit = 10 * lit + digit;
    }
    lit *= sign;

    // Skip whitespace before weight
    while (isspace(ch)) ch = parse_char();

    // Parse weight as string (to pass into GMP parser)
    std::string weight_str;
    if (!(isdigit(ch) || ch == '.' || ch == '/' || ch == 'e' || ch == 'E' || ch == '+' || ch == '-'))
      PER("expected decimal number for weight");

    while (isdigit(ch) || ch == '.' || ch == '/' || ch == 'e' || ch == 'E' || ch == '+' || ch == '-') {
      weight_str.push_back((char) ch);
      ch = parse_char();
    }
    // Optional: strip trailing comment
  ...
  // For all variables, check if positive occurrence has higher weight than negative occurrence.
  // If so, multiply internal->best_weight_residual by that weight.
  for (int var = 1; var <= max_var; ++var) {
    mpf_class pos_it = weights_map[var];
    mpf_class neg_it = weights_map[-var];
    if (pos_it > neg_it) {
      internal->log_best_weight_residual += std::log10(pos_it.get_d());
      internal->best_weight_residual *= pos_it;
  ...
  std::vector<std::pair<int, mpf_class>> sorted_literals(weights_map.begin(), weights_map.end());

  // Step 2: Sort by weight (mpf_class value)
  std::sort(sorted_literals.begin(), sorted_literals.end(),
      [](const std::pair<int, mpf_class>& a, const std::pair<int, mpf_class>& b) {
          return a.second < b.second;  // Ascending order
      });

  for (const auto& p : sorted_literals)
    internal->sorted_lits.push_back(p.first);
