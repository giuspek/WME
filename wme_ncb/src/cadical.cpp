...
int App::main (int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp (argv[i], "-h") || !strcmp (argv[i], "--help") ||
      ...
      if (++i == argc)
        APPERR ("argument to '--weights' missing");
      else if (weights_path)
        APPERR ("multiple weights files '--weights %s' and '--weights %s'",
                weights_path, argv[i]);
      else{
        weights_path = argv[i];
      }
      ...
      if (++i == argc)
        ...
        if (!parse_int_str(argv[i], topk_value) || topk_value == 0)
          APPERR ("invalid argument in '--topk %s' (expected positive integer)", argv[i]);

        solver->internal->topk = topk_value;
      }
    }
  ...
  if (!get ("quiet")) {
    ...
    solver->message ("%sWME - Non-chronological version%s",
  ...
  if (weights_path) {
    solver->section ("parsing weights");
    solver->message ("reading weights file from '%s'", weights_path);
    if ((err = solver->read_weights (weights_path)))
      APPERR ("%s", err);
    // You may want to store or use weights_map here as needed.
  } else {
    // Set the weight of each literal to 1 (no impact)
    for (int var = 1; var <= max_var; ++var) {
      solver->internal->weights_map[var] = mpf_class(1.0);
      solver->internal->weights_map[-var] = mpf_class(1.0);
    }
  }
  ...
  if (!weights_path) {
    // If no weights file is provided, set default weights
    for (int var = 1; var <= max_var; ++var) {
      solver->internal->weights_map[var] = mpf_class(1.0);
      solver->internal->weights_map[-var] = mpf_class(1.0);
  ...
  if (solver->internal->topk != 0 && solver->internal->topk_solver) {
    solver->internal->topk_solver->print();
    delete solver->internal->topk_solver;
    solver->internal->topk_solver = nullptr;
  }
  ...
  if (res == 10) {
    if (status)
      fprintf(write_result_file, "s %u\n", solver->internal->num_models);
