...
extern Clause backtrue_reason_clause;
extern Clause* backtrue_reason;
...
struct Internal {
  ...
  mpf_class weight;
  mpf_class weight_relevant;
  Assignment assignment;
};

struct TopK {
  size_t k;
  std::unordered_map<int, mpf_class> weights_map;
  Internal* internal_ref = nullptr;

  using Heap = std::priority_queue<
      ScoredAssignment,
      std::vector<ScoredAssignment>,
      std::greater<>>;

  struct MinCompare {
      bool operator()(const ScoredAssignment& a, const ScoredAssignment& b) const {
          return a.weight_relevant > b.weight_relevant; // mpf_class supports operator>
      }
  };

  std::priority_queue<ScoredAssignment, std::vector<ScoredAssignment>, MinCompare> heap;

  TopK(int k, const std::unordered_map<int, mpf_class>& weights_map)
      : k(k), weights_map(weights_map) {}

  mpf_class tryAdd(mpf_class weight, mpf_class weight_relevant, Assignment& assignment) {
    ScoredAssignment entry{weight, weight_relevant, assignment};

    if (heap.size() < k) {
        heap.push(entry);
    } else if (weight_relevant > heap.top().weight_relevant) {
        heap.pop();
        heap.push(entry);
    }

    if (heap.size() == k) {
      return heap.top().weight_relevant;
    }

    // Return a default value if the heap size is not equal to k
    return mpf_class(0);
  }

  std::vector<ScoredAssignment> getResults() const {
    std::vector<ScoredAssignment> results;
    auto copy = heap;
    while (!copy.empty()) {
  ...
  void print() {
    ...
    for (auto it = results.rbegin(); it != results.rend(); ++it) {
      printf("t ");
      for (const auto& lit : it->assignment) {
      printf("%d ", lit);
      }
      // Print weight in parenthesis with 2 decimal digits
      gmp_printf("(%.Ff) ", it->weight.get_mpf_t());
      long double log_sum = 0.0L;
      for (const auto& lit : it->assignment)  {
        long double w = weights_map[lit].get_d();
        if (w > 0.0L) {
            log_sum += std::log10(w);
        }
      }
  ...
  bool leq_guard_adaptive(const mpf_class& weight_temp,
                        const mpf_class& temp_residual,
                        const mpf_class& threshold)     // a couple of ULPs
  {
    mpf_class prod = weight_temp + temp_residual;

    // // precision (bits) of 'prod' (same as current default unless overridden)
    // const mp_bitcnt_t p = mpf_get_prec(prod.get_mpf_t());

    // // ulp-sized absolute epsilon: ulp_slack * 2^{-p}
    // mpf_class eps_prec = 1;
    // mpf_div_2exp(eps_prec.get_mpf_t(), eps_prec.get_mpf_t(), p); // eps_prec = 2^{-p}
    // eps_prec *= ulp_slack;

    // // adaptive relative scale: use max(threshold, 2^{-p})
    // mpf_class rel_anchor = (threshold > eps_prec) ? threshold : eps_prec;

    // // relative epsilon
    // mpf_class eps_rel = rel_anchor * rel_tol;

    // // final epsilon
    // mpf_class eps = (eps_prec > eps_rel) ? eps_prec : eps_rel;
  ...
  std::unordered_map<int, mpf_class> weights_map;
  mpf_class weight_assignment = mpf_class(1.0);
  mpf_class log_weight_assignment = mpf_class(0.0);
  mpf_class best_weight_residual = mpf_class(1.0);
  mpf_class log_best_weight_residual = mpf_class(0.0);
  mpf_class init_best_weight_residual = mpf_class(1.0);
  mpf_class log_init_best_weight_residual = mpf_class(0.0);
  mpf_class weight_assignment_relevant = mpf_class(1.0);
  mpf_class log_weight_assignment_relevant = mpf_class(0.0);
  bool w_heur_neg = true; // negative weight heuristic
  bool w_heur_pos = true; // positive weight heuristic
  bool w_heur_pos_high = true; // positive weight heuristic for high weights
  bool w_heur_prob = true; // positive weight heuristic for probabilities
  mpf_class threshold = mpf_class(0); // threshold for weight heuristics 
  bool weight_conflict_flag = false; // flag for weight conflict
  unsigned num_models = 0; // number of models found
  unsigned topk = 0;
  TopK *topk_solver = nullptr; // Top-K solver for weighted models
  bool weight_environment = false; // true if weight environment is used
  std::unordered_map<int,int> var_occurrences;
  std::unordered_set<int> is_relevant;
  int max_occurrence = 1; // avoid division by 0
  ...
  void reinit_scores (int old_max_var, int new_max_var);
  ...
  void get_weight_implication_reason();
...
inline bool score_smaller::operator()(unsigned a, unsigned b) {
  assert(1 <= a && a <= (unsigned) internal->max_var);
  assert(1 <= b && b <= (unsigned) internal->max_var);

  auto &weights_map = internal->weights_map;
  auto &stab = internal->stab;

  // Step 1: Prioritize relevant variables
  bool a_relevant = internal->is_relevant.count(a);
  bool b_relevant = internal->is_relevant.count(b);

  if (a_relevant && !b_relevant) return false;  // a is better
  if (!a_relevant && b_relevant) return true;   // b is better

  // // Step 2: Composite heuristic score (weighted)
  auto wa = std::max(weights_map[a], weights_map[-a]);
  auto wb = std::max(weights_map[b], weights_map[-b]);

  double wa_norm = wa.get_d();  // assuming in (0,1]
  double wb_norm = wb.get_d();

  double sa = stab[a], sb = stab[b];
  double max_stab = std::max(sa, sb);
  if (max_stab == 0.0) max_stab = 1.0;
  double sa_norm = sa / max_stab;
  double sb_norm = sb / max_stab;

  constexpr double alpha = 0.0;
  constexpr double beta = 0.2;
  constexpr double gamma = 1.0 - alpha - beta; // Not used in this version

  // Step 3: Use variable occurrence count as tie-breaker
  int occ_a = internal->var_occurrences[a];
  int occ_b = internal->var_occurrences[b];

  double max_occ = std::max(occ_a, occ_b);

  // double score_a = alpha * wa_norm + beta * sa_norm + gamma * (occ_a / max_occ);
  // double score_b = alpha * wb_norm + beta * sb_norm + gamma * (occ_b / max_occ);
  ...
  // if (occ_a < occ_b) return true;
  // if (occ_a > occ_b) return false;

  // Step 4: Final fallback — lexicographic order (higher var index first)
  return a > b;
}
