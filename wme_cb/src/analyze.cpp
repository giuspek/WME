...
// static Clause backtrue_reason_clause;
// static Clause *backtrue_reason = &backtrue_reason_clause;
...
inline void Internal::bump_clause (Clause *c) {
  LOG (c, "bumping");
  if (c == backtrue_reason) {
    return; // do not bump the backtrue reason
  }
...
inline void Internal::analyze_reason (int lit, Clause *reason, int &open,
  ...
  if (reason == backtrue_reason) {
    // Collect all decision literals up to the current literal (excluding lit itself)
    // Collect all decision literals up to the current literal (excluding lit itself)
    // and all literals at level 0 (excluding lit itself) in a single loop over the trail
    for (const auto &trail_lit : trail) {
      if (trail_lit == lit) continue;
      int lvl = var(trail_lit).level;
      if (lvl == 0 || (lvl > 0 && control[lvl].decision == trail_lit)) {
        analyze_literal(lvl ? -trail_lit : trail_lit, open, resolvent_size, antecedent_size);
      }
    }
    // Finally, analyze the literal itself
    analyze_literal(lit, open, resolvent_size, antecedent_size);
    return;
  }
  else{
...
void Internal::learn_blocking_clause (int forcing_lit) {
  // We learn a blocking clause to prevent the same conflict from happening
  // again. This is a special case of a driving clause.
  original.clear ();
  for (const auto &trail_lit : trail) {
    int lvl = var(trail_lit).level;
    if (abs(forcing_lit) == abs(trail_lit)) {
      continue;
    }
    if (control[lvl].decision == trail_lit) {
      add_original_lit(-trail_lit);
    } 
  }
  if (original.size() == 1) {
    // If we only have one decision literal, we can just unit propagate it
    backtrack();
    int unit_lit = original[0];
    assign_unit(unit_lit);
    original.clear();
    return;
  }
}
...
void Internal::get_weight_implication_reason () {
  mpf_class weight_temp = 0.0;
  mpf_class temp_residual = log_init_best_weight_residual;
  int max_level = 0;

  int counter = 1;
  clause.clear ();
  for (auto it = sorted_lits.begin(); it != sorted_lits.end(); ++it) {
    int lit = *it;
    if (is_relevant.find(abs(lit)) == is_relevant.end()) continue;
    auto trail_it = std::find(trail.begin(), trail.end(), lit);
    if (trail_it != trail.end()) {
      weight_temp += std::log10(weights_map[lit].get_d());
      temp_residual -= std::log10(std::max(weights_map[lit].get_d(), weights_map[-lit].get_d()));
      clause.push_back(-lit);
      max_level = std::max(max_level, var(lit).level);
      if (leq_guard_adaptive(weight_temp, temp_residual, threshold)) {
        break;
      }
    }
  }
  ...
  if (max_level == 0) {
    // All literals in the clause are at level 0, so we have derived an empty clause
    learn_empty_clause();
    conflict = 0;
    return;
  }
  ...
  if (clause.size() == 1){
    if (var(clause[0]).level == 0) {
      learn_empty_clause();
      conflict = 0;
      return;
    }
    else{
      int level_to_backtrack = var(clause[0]).level;
      int decision_var = control[level_to_backtrack].decision;
      // get decision variable at that level
      backtrack(level_to_backtrack-1);
      search_assign_driving(-decision_var, backtrue_reason);
      conflict = 0;
      return;
    }

    std::vector<int> negation_clause;
    // LEARN A BLOCKING CLAUSE AS NEGATION OF ALL DECISION LITERALS UP TO THIS POINT
    for (int lvl = 1; lvl <= level; ++lvl) {
      int decision_lit = control[lvl].decision;
      negation_clause.push_back(-decision_lit);
    }
  
    std::vector<int> unit_chain_back;
    std::vector<bool> unit_chain_back_reasons; // 0 if decision, 1 if backtrue
    int last_decision_to_flip = 0;
    // Scan the trail to collect all literals whose reason is either a decision or backtrue reason
    for (const auto &trail_lit : trail) {
      if (abs(trail_lit) == abs(clause[0])) continue; // skip the unit literal itself
      if (var(trail_lit).reason == backtrue_reason) {
        unit_chain_back.push_back(trail_lit);
        unit_chain_back_reasons.push_back(true); // 1 if backtrue
      } else if (control[var(trail_lit).level].decision == trail_lit) {
        unit_chain_back.push_back(trail_lit);
        unit_chain_back_reasons.push_back(false); // 0 if decision
        last_decision_to_flip = trail_lit;
      }
    }
    backtrack (0);
    ...
    // ADD ALL ELEMENTS IN NEGATED CLAUSE TO clause
    for (const auto &lit : negation_clause) {
      clause.push_back(lit);
    }
  ...
  Clause *new_conflict = new_learned_redundant_clause(counter);
  new_conflict->used = 1 + (counter <= opts.reducetier2glue);

  conflict = new_conflict;

  if (var(clause[0]).level == var(clause[1]).level){
    // Simply chronologically backtrack and flip the last decision literal
    int flipped_literal = -control[var(clause[0]).level].decision;
    LOG("flipping last decision literal %d", flipped_literal);
    backtrack(var(clause[0]).level - 1);
    search_assign_driving(flipped_literal, backtrue_reason);
    conflict = 0;
    return;
  }
...
void Internal::analyze () {
  ...
  if (weight_conflict_flag) {
    if (level == 0){
      return;
    }
    assert(level > 0);
    int flipped_literal = -control[level].decision;
    get_weight_implication_reason();
    weight_conflict_flag = false;
    if (conflict == 0){
      STOP(analyze);
      return;
    }
  ...
  for (;;) {
    ...
    while (!uip) {
      assert (i > 0);
      const int lit = (*t)[--i];
      if (lit == control[level].decision)
        uip = lit; // if we are on decision level, we can use the decision
      if (!flags (lit).seen)
        continue;
  ...
  if (clause.size() == 1){
    int decision_lit = control[level].decision;
    backtrack (level - 1);
    LOG ("search assign driving %d as unit", decision_lit);
    search_assign_driving (-decision_lit, backtrue_reason);
    return;
  }
  ...
  if (clause.size () == 1) {
    if (var(clause[0]).level == 0) {
      ...
      int var_var = clause[0];
      learn_blocking_clause(clause[0]);
      backtrack (0);
      if (val(clause[0]) == 0)
        assign_unit (var_var);
    }
  ...
  bool cannot_be_unit_propagated = var(clause[0]).level == var(clause[1]).level;
  int jump;
  Clause *driving_clause = new_driving_clause (glue, jump);
  UPDATE_AVERAGE (averages.current.jump, jump);

  int new_level = 0;
  if (cannot_be_unit_propagated) 
    new_level = level;
  else
    new_level = determine_actual_backtrack_level (jump);
  ...
  if (uip) {
    if (cannot_be_unit_propagated) {
      LOG ("search assign driving %d as unit", uip);
      backtrack (level - 1);
      search_assign_driving (-uip, backtrue_reason);
    } else {
      LOG ("search assign driving %d", uip);
