...
void Internal::reinit_scores (int old_max_var, int new_max_var) {
  LOG ("reinitializing EVSIDS scores from %d to %d", old_max_var + 1,
       new_max_var);
  scores.clear ();
  for (int i = old_max_var; i < new_max_var; i++){
    scores.push_back(i + 1);
  ...
  // scores.build_heap(); // rebuild heap completely
}
