...
// Similar to 'decision_reason', we use the address of 'backtrue_reason' as a pseudo reason
// to distinguish assignments made due to backtracking-true (backtrue) steps from other assignments.
// This allows us to identify backtrue assignments during analysis or debugging, as they will have
// 'backtrue_reason' as their reason pointer.

// This is useful in advanced backtracking strategies or when tracking the origin of assignments
// is necessary for proof logging or debugging.
...
void Internal::backtrue_flip (int lit) {
  assert (lit);
  assert (val (lit) < 0);
  LOG ("backtrue flip %d", lit);
  // We do not bump the reason literals here, because this is a special
  // case of a backtracking step and we do not want to bump the reasons
  // of the literals assigned by this step.
  search_assign (lit, backtrue_reason);
}
...
void Internal::assign_score(int lit){
  if (weight_environment){
    ...
    if (is_relevant.find(abs(lit)) != is_relevant.end()) {
      ...
      best_weight_residual /= std::max(weights_map[lit], weights_map[-lit]);
      log_best_weight_residual -= std::log10(std::max(weights_map[lit], weights_map[-lit]).get_d());
    }
  }
}
...
inline void Internal::search_assign (int lit, Clause *reason) {
  ...
  assert (!flags (idx).eliminated () || reason == decision_reason ||
          reason == external_reason || reason == backtrue_reason);
  Var &v = var (idx);
  int lit_level;
  assert (!lrat || level || reason == external_reason ||
          reason == decision_reason || reason == backtrue_reason || !lrat_chain.empty ());
  ...
  if (!reason){
    ...
    lit_level = level, reason = backtrue_reason; // backtrue assignment
  }
  else if (opts.chrono)
...
bool Internal::check_weight_conflict() {
  // Check if the current weight assignment leads to a conflict.
  ...
  if (w_heur_prob && internal->log_weight_assignment_relevant + internal->log_best_weight_residual <= internal->threshold) {
    // If the weight assignment is below the threshold, we consider it a conflict.
    weight_conflict_flag = true;
    return true;
  }
  return false;
}
...
bool Internal::propagate () {
  ...
  // Pre-check if weight already leads to a conflict.
  if (check_weight_conflict()) {
    LOG ("weight conflict detected, flipping last decision literal");
    conflict = backtrue_reason;
