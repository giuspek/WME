...
Clause backtrue_reason_clause;
Clause *backtrue_reason = &backtrue_reason_clause;
...
void Internal::print_current_model() {
  ...
  for (size_t i = 0; i < trail.size(); ++i) {
    ...
    if (var(trail[i]).reason == backtrue_reason) {
      printf("*");
    }
    printf("%d ", lit);
  }
  gmp_printf(" (%.Ff) ", weight_assignment.get_mpf_t());
...
void Internal::weight_pruning(){
  int first_not_relevant_level = level;
  for (int lvl = 1; lvl <= level; ++lvl) {
    int decision_lit = control[lvl].decision;
    if (decision_lit != 0 && is_relevant.find(abs(decision_lit)) == is_relevant.end()) {
      first_not_relevant_level = lvl;
      break;
    }
  }
  // first_not_relevant_level now holds the level of the first not relevant decision literal, or -1 if all are relevant
  backtrack(first_not_relevant_level);

  mpf_class temp_best_residual_weight = best_weight_residual;
  mpf_class temp_weight_assignment = weight_assignment_relevant;

  int second_not_relevant_level = 0;

  for (int i = (int)trail.size() - 1; i >= 0; --i) {
    int lit = trail[i];
    if (is_relevant.find(abs(lit)) != is_relevant.end()) {
      mpf_class w = std::max(weights_map[lit], weights_map[-lit]);
      temp_best_residual_weight *= w;
      temp_weight_assignment /= weights_map[lit];
      if (temp_weight_assignment * temp_best_residual_weight > threshold) {
        second_not_relevant_level = std::max(second_not_relevant_level, var(lit).level);
        if (var(lit).reason == 0)
          break;
      }
    }
...
void Internal::weight_pruning_conflict(){
  ...
  for (int i = (int)trail.size() - 1; i >= 0; --i) {
    ...
    if (is_relevant.find(abs(lit)) != is_relevant.end()) {
      ...
      temp_weight_assignment /= weights_map[lit];
      if (temp_weight_assignment * temp_best_residual_weight >= threshold) {
        second_not_relevant_level = var(lit).level;
        break;
      }
    }
  }
...
void Internal::block_last_decision_path() {
  ...
  int idx = control[level].trail;
  int flipped_lit = -(trail[idx]);

  // Backtrack to previous level
  if (level > 0) backtrack(level - 1);

  backtrue_flip(flipped_lit);
}
...
int Internal::cdcl_loop_with_inprocessing () {
  ...
  if (topk > 0) {
    topk_solver = new TopK(topk, weights_map);
  } 

  reinit_scores (0, max_var);

for (int idx = 1; idx <= max_var; idx++) {
    if (var_occurrences[idx] > max_occurrence)
        max_occurrence = var_occurrences[idx];
}
  ...
  while (!res) {
    if (unsat)
      ...
      if (level == 0) {
        // If we are at the root level and no conflict, we can stop
