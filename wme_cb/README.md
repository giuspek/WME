...
# WME_CB — Weighted Model Enumeration with Non-Chronological Backtracking

This repository contains **WME_CB**, a **Weighted Model Enumeration (WME)** solver built on top of a lightly modified **CaDiCaL** backend.
...
```bash
build/wme_cb [WME options] input.cnf
```
...
```bash
build/wme_cb --weights weights.txt input.cnf
```

### 3. Return only the top-\(k\) models

```bash
build/wme_cb --weights weights.txt --topk 10 input.cnf
```

### 4. Enumerate only models above a threshold

```bash
build/wme_cb --weights weights.txt --logthreshold -3 input.cnf
```

or equivalently, if you prefer specifying the threshold in the original scale:

```bash
build/wme_cb --weights weights.txt --threshold 0.001 input.cnf
```
...
```bash
build/wme_cb --weights weights.txt --topk 5 input.cnf
```
...
```bash
build/wme_cb --weights weights.txt --threshold 0.01 input.cnf
```
...
```bash
build/wme_cb --weights weights.txt --logthreshold -2 input.cnf
```
...
```bash
build/wme_cb --weights weights.txt --topk 20 input.cnf
```
...
```bash
build/wme_cb --weights weights.txt --threshold 1e-4 input.cnf
```
